class Solution {
    ...
    //Complexities:
    // - space: O(1)
    // - time: O(n + m) + hashing costs

    //Optimize:
    // - since we only use lowercase english alphabet letters we can use an array
    // - create single int arr with 26 entries
    // - iterate through both s and t
    // - - for s, increment position i - 'a'
    // - - for t, decrement position i - 'a'
    // - then iterate through whole int arr, if any entry != 0 -> return false
    // - else -> return true

    //Complexities:
    // - space: O(1)
    // - time: O(n) without hashing costs
